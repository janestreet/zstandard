...
module C(F: Cstubs.FOREIGN) = struct
  ...
  let compress = foreign "ZSTD_compress"
                   (ptr void  (* dst *)
                    @-> size_t (* dstCapacity *)
                    @-> const (ptr void) (* src *)
                    @-> size_t (* srcSize *)
  ...
  let decompress = foreign "ZSTD_decompress"
                     ( ptr void (* dst *)
                       @-> size_t (* dstCapacity *)
                       @-> const (ptr void) (* src *)
  ...
  let getFrameContentSize =
    foreign "ZSTD_getFrameContentSize" (
      const (ptr void) (* src *)
      @-> size_t (* srcSize *)
      @-> returning Ctypes.ullong
    )
  ;;

  let compressBound =
    foreign "ZSTD_compressBound" (
      size_t (* srcSize *)
      @-> returning size_t
    )
  ;;
  ...
  let getErrorName =
    foreign "ZSTD_getErrorName" (
      size_t (* code *)
      @-> returning (const (ptr char))
    )
  ;;

  let maxCLevel =
    foreign "ZSTD_maxCLevel" (
      void
      @-> returning int
    )
  ;;
  ...
  module Context = struct
    module Compression = struct
      ...
      let compress = foreign "ZSTD_compressCCtx" (
        ptr t
        @-> ptr void  (* dst *)
        @-> size_t (* dstCapacity *)
        @-> const (ptr void) (* src *)
    ...
    module Decompression =  struct
      ...
      let decompress = foreign "ZSTD_decompressDCtx" (
        ptr t
        @-> ptr void  (* dst *)
        @-> size_t (* dstCapacity *)
        @-> const (ptr void) (* src *)
  ...
  module Simple_dictionary = struct
    let compress_usingDict = foreign "ZSTD_compress_usingDict" (
    ...
    let decompress_usingDict = foreign "ZSTD_decompress_usingDict" (
  ...
  module Bulk_processing_dictionary = struct
    module Compression = struct
      ...
      let create = foreign "ZSTD_createCDict" (
      ...
      let compress = foreign "ZSTD_compress_usingCDict" (
    ...
    module Decompression = struct
      ...
      let create = foreign "ZSTD_createDDict" (
        const (ptr void) (* dictBuffer *)
        @-> size_t (* dictSize *)
        @-> returning (ptr t)
      )
      let free = foreign "ZSTD_freeDDict" (ptr t @-> returning size_t)
      let decompress = foreign "ZSTD_decompress_usingDDict" (
  ...
  module Streaming = struct
    ...
    module Compression = struct
      ...
      let compress = foreign "ZSTD_compressStream" (
        ptr cctx
        @-> ptr outbuffer
        @-> ptr inbuffer
        @-> returning size_t
      )
      let flushStream = foreign "ZSTD_flushStream" (
        ptr cctx
        @-> ptr outbuffer
        @-> returning size_t
      )
      let endStream = foreign "ZSTD_endStream" (
        ptr cctx
        @-> ptr outbuffer
        @-> returning size_t
      )
    ...
    module Decompression = struct
      ...
      let decompress = foreign "ZSTD_decompressStream" (
        ptr dctx
        @-> ptr outbuffer
        @-> ptr inbuffer
        @-> returning size_t
